/*
 ...
 -----  ---------   ---------------------------------------------
   ...
   10   25-Sep-14   - Network Context support   
        06-Feb-15   - Enable QOS (-q option) to be set to 0x00.
 ...
 */
#ifdef LINT_ARGS
static int MTU_recover_param(u8 pname, u8 plen, u8 *pptr, SMSR_MSG *ind);
#else
 ...
 * Always returns zero.
 */
int MTU_dlg_req_to_msg(m, dlg_req)
  MSG *m;
  SMSR_MSG *dlg_req;
{
  ...
  *pptr++ = (u8)dlg_req->type;
   ...
   * NC
   */
  if (bit_test(dlg_req->pi, MAPPN_nc))
  {
    *pptr++ = MAPPN_LONG_PARAM_CODE_EXTENSION;
    *pptr++ = 4;
    *pptr++ = (MAPPN_nc >> 8);        
    *pptr++ = (MAPPN_nc & 0xff);    
    *pptr++ = 1;    
  ...
  /*
   * QoS - insert if required by command line option
   * bit0 = return option, bit1 = class
   * Note that MAP sets 0x2 by default (i.e. class 1) if not added here
   * Also 2 optional octets may be present
   */
  if (bit_test(dlg_req->pi, MAPPN_qos))
  {
    *pptr++ = MAPPN_qos;
    *pptr++ = dlg_req->qos_size;

    for(i = dlg_req->qos_size-1; i>=0; i--)
      *pptr++ = (dlg_req->qos >> (i*8)) & 0xff;

    mlen += 2 + dlg_req->qos_size;
  }
 ...
 * Always returns zero.
 */
int MTU_srv_req_to_msg(m, srv_req)
  MSG *m;
  SMSR_MSG *srv_req;
{
  int   mlen;           /* total param length */
  ...
  *pptr++ = (u8)srv_req->type;
  ...
  if (bit_test(srv_req->pi, MAPPN_msisdn))
  {
      ...
      printf("here in msisdn processing\n");
    *pptr++ = MAPPN_msisdn;
int ret = srv_req->msisdn.num_dig_bytes + 1;
 printf("ret msisdn = %d\n", ret);
 ...
 */
int MTU_msg_to_ind(ind, m)
  SMSR_MSG  *ind;        /* structure to recover data to */
 ...
 */
static int MTU_recover_param(pname, plen, pptr, ind)
  u8 pname;             /* parameter name */
  u8 plen;              /* parameter length */
  u8 *pptr;             /* parameter data */
  SMSR_MSG *ind;         /* structure to map parameter into */
{
  u8 i;                 /* loop counter */
 ...
 */
u8 MTU_str_to_def_alph(ascii_str, da_octs, da_len, max_octs)
 ...
 */
u8 MTU_USSD_str_to_def_alph(ascii_str, da_octs, da_len, max_octs)
  char *ascii_str;      /* The source of the ascii chars */
  u8   *da_octs;        /* The u8 array into which the deft alph char are built */
  u8  *da_len;         /* The formated octet length of da_octs, updated */
  u8   max_octs;       /* The max size of da_octs */
